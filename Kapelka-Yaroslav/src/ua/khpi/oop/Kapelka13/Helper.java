...
public class Helper
{
	...
	static void TransitRoutes(MyCollection<Route> collection) 
	{
		...
		{
			if((rStation.matcher(value.getStation_name())).find() && (rStatus.matcher(value.getStatus_station())).find() && (rDepartureTime.matcher(value.getDeparture_time())).find() && (rArrivalTime.matcher(value.getArrival_time())).find())
			{
...
class ThreadFindAspects extends Thread {
	...
	public void run() {
		...
		System.out
				.println(str + count + "\nПрошло миллисекунд с начала выполнения: " + (System.currentTimeMillis() - m));
